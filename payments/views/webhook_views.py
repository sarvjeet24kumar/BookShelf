...
class RazorpayWebhookView(APIView):
    ...
    def post(self, request):
        ...
        if not payment_id:
            ...
            return Response(
                {"error": "Missing payment_id"}, status=status.HTTP_400_BAD_REQUEST
            )

        if not order_id:
            logger.error("Webhook missing order reference")
            return Response(
                {"error": "Missing order_id"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Identify tenant for logging context
        try:
            payment = Payment.all_objects.select_related("tenant").get(
                razorpay_order_id=order_id
            )
            set_current_tenant(payment.tenant)
        except Payment.DoesNotExist:
            logger.warning(
                f"Tenant identification failed: Payment not found for order {order_id}"
            )
        ...
        if isinstance(razorpay_created_at, int):
            data["created_at"] = datetime.fromtimestamp(
                razorpay_created_at, tz=timezone.UTC
            ).isoformat()
        ...
        if not event_type:
            ...
            return Response(
                {"error": "Malformed payload - missing event type"},
                status=status.HTTP_400_BAD_REQUEST,
            )
