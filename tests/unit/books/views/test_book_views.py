...
class TestBookViewGet:
    ...
    def test_list_books_admin_cache_miss(
        self, MockBook, mock_cache_svc, MockFilter, MockSerializer,
        list_view, factory, mock_admin
    ):
    ...
    def test_list_books_cache_hit(
        ...
        cached_data = [{"id": str(fake_data.uuid4()), "title": fake_data.sentence(nb_words=3)}]
...
class TestBookViewPost:
    ...
    def test_create_book_success(
        self, MockCreateSerializer, MockListSerializer, list_view, factory, mock_user, fake_data
    ):
    ...
    @patch("books.views.book_views.BookCreateSerializer")
    def test_create_book_invalid_data(self, MockSerializer, list_view, factory, mock_user):
...
class TestBookDetailView:
    ...
    @patch("books.views.book_views.BookListSerializer")
    @patch("books.views.book_views.Book")
    def test_get_book_as_admin(self, MockBook, MockSerializer, detail_view, factory, mock_admin, fake_data):
        ...
        mock_ser = MockSerializer.return_value
        mock_data = {"id": str(fake_data.uuid4()), "title": fake_data.sentence(nb_words=3)}
    ...
    @patch("books.views.book_views.Book")
    def test_delete_book_non_admin_blocked(self, MockBook, MockUserBook, detail_view, factory, mock_user):
    ...
    @patch("books.views.book_views.UserBook")
    @patch("books.views.book_views.Book")
    def test_delete_book_success(self, MockBook, MockUserBook, detail_view, factory, mock_admin):
    ...
    def test_patch_book_non_admin_non_owner_blocked(
        self, MockBook, MockUpdateSer, MockListSer, detail_view, factory, mock_user, fake_data
    ):
        ...
        request = factory.patch("/api/v1/books/1/", {"title": fake_data.sentence(nb_words=3)}, format="json")
        force_authenticate(request, user=mock_user)
        response = detail_view(request, id="1")
        assert response.status_code == status.HTTP_403_FORBIDDEN
