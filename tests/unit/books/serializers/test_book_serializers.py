import pytest
from books.serializers.book_serializers import BookCreateSerializer, BookUpdateSerializer
...
class TestBookCreateSerializerUnit:
    ...
    def test_validate_genres_empty(self):
        ...
        assert "At least one genre is required" in str(serializer.errors.get("genres", []))
    ...
    def test_validate_genres_non_existent(self, db, fake_data):
        ...
        assert "IDs not found" in error_str or "None of the provided genre IDs exist" in error_str
...
class TestBookUpdateSerializerUnit:
    ...
    def test_non_admin_cannot_change_status(self):
        ...
        serializer = BookUpdateSerializer(data=data, context={"request": mock_request}, partial=True)
        assert not serializer.is_valid()
        assert "Only admins can change the request status" in str(serializer.errors.get("request_status", []))
